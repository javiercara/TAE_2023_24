...
Sin embargo no podemos utilizar la distribución de $y_p$ ya que desconocemos $\beta$ y $\sigma^2$. La opción es trabajar con el error de predicción. Se define el error de predicción como la diferencia entre $y_p$ y su valor medio predicho $\hat{y}_p$:

$$
\epsilon_p = y_p - \hat y_p = y_p - x_p^T \hat \beta
$$

La variable aleatoria $\epsilon_p$ no es un residuo. Los residuos se definen para datos observados. Hemos visto que
...
En este caso no se puede utilizar bootstrap. El método bootstrap se utiliza para calcular intervalos de confianza. Pero $y_p$ es una variable aleatoria y el intervalo de predicción no es un intervalo de confianza. 